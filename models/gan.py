...
class DefenseGANBase(AbstractModel):
    def __init__(self, cfg=None, test_mode=False, verbose=True, **args):
        default_attributes = ['dataset_name', 'batch_size', 'use_bn',
                              ...
                              'attribute', 'model_type', 'model_folder', 'defense_type', 'nb_classes']
        ...
        self.new_batch_size = self.batch_size
        ...
        self._load_dataset()
        # self.____local = tf.local_variables()
        # self.sess.run(tf.variables_initializer(self.____local))
    ...
    def _build(self):
        ...
        if self.defense_type == 'none0': #
            self.fake_data = self.generator_fn()
        else:
            models = {'A': model_a, 'B': model_b, 'C': model_c, 'D': model_d, 'E': model_e, 'F': model_f}
            self.model = models[self.model_type](input_shape=[None] + self.image_dim, nb_classes=self.nb_classes)
            
            classifier_folder = os.path.join(self.model_folder, self.model_type +'/')
            saver = tf.train.Saver()
            if os.path.isfile(os.path.join(classifier_folder,'classifier.ckpt.index')):
                #load model
                saver.restore(self.sess, os.path.join(classifier_folder, 'classifier.ckpt'))

            self.attack_obj = MomentumIterativeMethod(self.model, sess=self.sess)
            if '1' in self.defense_type:
                self.recon_data = self.real_data
            else:
                self.recon_data = self.reconstruct(self.real_data, batch_size=self.batch_size, \
                    back_prop=True, z_init_val=None, reconstructor_id=123)
            self.attack_params = {'eps': 0.3, 'ord': np.inf, 'clip_min': 0., 'clip_max': 1.} #
            self.adv_x = tf.stop_gradient(self.attack_obj.generate(self.recon_data, **self.attack_params) - \
                self.recon_data + self.real_data)

            self.real_data_ = self.real_data
            self.fake_data_ = self.generator_fn()
            self.real_data  = tf.tile(self.real_data_, multiples=[2, 1, 1, 1])
            self.fake_data  = tf.concat([self.fake_data_, self.adv_x], axis=0)
    ...
    def _loss(self):
        ...
        if self.mode == 'wgan':
            ...
            if self.defense_type == 'none0': #
                ...
                self.new_batch_size = 2 * self.batch_size

            alpha = tf.random_uniform(
                shape=[self.new_batch_size, 1, 1, 1],
                minval=0.,
                maxval=1.
            )
    ...
    def train(self, phase=None, output=None):
        ...
        for iteration in range(cur_iter, max_train_iters):
            ...
            if iteration > 0 and 'gan' in self.mode and phase is None:
                ...
                _ = sess.run(self.gen_train_op,
                             feed_dict={self.real_data_pl: next(gen), \
                                        self.is_training: 1})
    ...
    def save_ds(self):
        ...
        for split in splits:
            ...
            if os.path.exists(orig_imgs_pkl_path) and not self.test_again:
                with open(orig_imgs_pkl_path, 'rb') as f:
                    all_recs = cPickle.load(f)
                    could_load = True
            ...
            with open(orig_imgs_pkl_path, 'wb') as f:
                cPickle.dump(orig_imgs, f, -1)
                cPickle.dump(all_targets, f, -1)
...
class MnistDefenseGAN(DefenseGANBase):
    ...
    def generate_image(self, iteration, path=None):
        samples = self.sess.run(self.fixed_noise_samples)
