...
from cleverhans.attacks import FastGradientMethod, MomentumIterativeMethod, MadryEtAl
...
def whitebox(gan, rec_data_path=None, batch_size=128, learning_rate=0.001,
    ...
    alters_pl = tf.placeholder(tf.float32, shape=[None] + list(train_images.shape[1:]))
    ...
    eval_params = {'batch_size': batch_size}
    acc = model_eval(
        sess, images_pl, labels_pl, preds, test_images, test_labels,
    ...
    if 'bpda' in FLAGS.attack_type: #
        ...
        if '1' in FLAGS.attack_type:
            ...
            attack_obj = MomentumIterativeMethod(model, sess=sess)

        if FLAGS.defense_type == 'defense_gan': # 2
    ...
    if FLAGS.defense_type == 'defense_gan':
        ...
        if 'bpda' in FLAGS.attack_type:
            ...
            preds_adv = model.get_probs(irecon_adv_x)
        ...
        acc_adv, roc_info = model_eval_gan(
            ...
            feed={K.learning_phase(): 0}, diff_op=diff_op, attack=('bpda' in FLAGS.attack_type),
            alter=alters_pl, adv_samples=adv_x)
        ...
        #             os.path.join('images_defgan_35', 'samples_{}_{}_{}.png'.format(FLAGS.model, FLAGS.fgsm_eps, j))
        #         )
...
if __name__ == '__main__':
    ...
    flags.DEFINE_string("model_folder", "./trained_classifier/", "Folder with trained classifiers")
