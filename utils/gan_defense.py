...
def model_eval_gan(
    ...
    for batch in range(nb_batches):
        ...
        feed_dict = {images: batch_images, labels: test_labels[start:end]}
        if feed is not None:
            feed_dict.update(feed)

        if attack:
            for i in range(10 - 1): #
                temp_input = sess.run(adv_samples, feed_dict=feed_dict)
                feed_dict.update({images: temp_input})
                sess.run(tf.local_variables_initializer())
            temp_input = sess.run(adv_samples, feed_dict=feed_dict)
            feed_dict.update({images: batch_images, alter: temp_input})
            sess.run(tf.local_variables_initializer())
